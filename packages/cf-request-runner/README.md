...
Every command first checks npm for a newer `@saptools/cf-request-runner` (at most once an hour, one small request
...
```text
cf-request-runner: updating 0.3.0 -> 0.4.0 ...
cf-request-runner: updated to 0.4.0; re-running the command
```
...
`npm install -g @saptools/cf-request-runner@latest` to update by hand.
...
| `CF_REQUEST_RUNNER_AUTO_UPDATE` | same values, this CLI only; wins over the global variable |
