...
function renderLifecycleResult(value: Record<string, unknown>): string {
  return `${value["status"] as string}: ${value["message"] as string}`;
}
...
function renderSessionStatus(value: Record<string, unknown>): string {
  return [
    `sessionId: ${formatCell(value["sessionId"])}`,
    `status: ${formatCell(value["status"])}`,
    `brokerAlive: ${formatCell(value["brokerAlive"])}`,
    `sshAlive: ${formatCell(value["sshAlive"])}`,
    `socketAlive: ${formatCell(value["socketAlive"])}`,
  ].join("\n");
}
...
function renderSessionRecord(value: Record<string, unknown>): string {
  return [
    `sessionId: ${formatCell(value["sessionId"])}`,
    `status: ${formatCell(value["status"])}`,
    `brokerPid: ${formatCell(value["brokerPid"])}`,
    `socketPath: ${formatCell(value["socketPath"])}`,
  ].join("\n");
}
