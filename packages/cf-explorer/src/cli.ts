...
function addSessionCommands(program: Command): void {
  ...
  addTargetOptions(session.command("start").description("Start a persistent explorer session"))
    .option("--timeout <seconds>", "Startup timeout in seconds")
    .action(async (flags: TargetFlags): Promise<void> => {
      writeOutput(await startExplorerSession({ ...buildDiscovery(flags), runtime: buildRuntime(flags) }));
    });
