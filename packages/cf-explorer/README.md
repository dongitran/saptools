...
Every command first checks npm for a newer `@saptools/cf-explorer` (at most once an hour, one small request
...
```text
cf-explorer: updating 0.5.0 -> 0.6.0 ...
cf-explorer: updated to 0.6.0; re-running the command
```

If the install cannot complete, one stderr line gives the manual command and the command runs on the
installed version; that version is not retried for a day. `cf-explorer self-update` forces the check and
install now; `cf-explorer self-update --check` only reports.
...
| `CF_EXPLORER_AUTO_UPDATE` | same values, this CLI only; wins over the global variable |
