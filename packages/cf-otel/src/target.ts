...
export async function resolveTarget(opts: TargetOptions): Promise<ResolvedTarget> {
  ...
  if (region !== undefined && org !== undefined && space !== undefined) {
    ...
    if (apiEndpoint === undefined) {
      throw new CfOtelError("TARGET_UNRESOLVED", `Unknown SAP CF region "${region}"`);
    }
  ...
  if (apiEndpoint === undefined) {
    throw new CfOtelError("TARGET_UNRESOLVED", `Unknown SAP CF region "${resolvedRegion}"`);
  }
...
/** Print the same style of resolved-target notice `cf-hana` prints, adapted to three flags. */
