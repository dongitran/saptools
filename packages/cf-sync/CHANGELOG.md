...
- **Self-updating.** Every command now checks npm for a newer `@saptools/cf-sync` (at most once an hour)
  ...
  off by itself in CI, in tests, from a source checkout and inside the re-run. New `cf-sync self-update
