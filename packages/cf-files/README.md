...
Every command first checks npm for a newer `@saptools/cf-files` (at most once an hour, one small request
...
```text
saptools-cf-files: updating 0.4.0 -> 0.5.0 ...
saptools-cf-files: updated to 0.5.0; re-running the command
```

If the install cannot complete, one stderr line gives the manual command and the command runs on the
installed version; that version is not retried for a day. `saptools-cf-files self-update` forces the check and
install now; `saptools-cf-files self-update --check` only reports.
...
| `CF_FILES_AUTO_UPDATE` | same values, this CLI only; wins over the global variable |
