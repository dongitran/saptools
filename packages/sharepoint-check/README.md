...
Every command first checks npm for a newer `@saptools/sharepoint-check` (at most once an hour, one small request
...
```text
saptools-sharepoint-check: updating 0.2.0 -> 0.3.0 ...
saptools-sharepoint-check: updated to 0.3.0; re-running the command
```

If the install cannot complete, one stderr line gives the manual command and the command runs on the
installed version; that version is not retried for a day. `saptools-sharepoint-check self-update` forces the check and
install now; `saptools-sharepoint-check self-update --check` only reports.
...
| `SHAREPOINT_CHECK_AUTO_UPDATE` | same values, this CLI only; wins over the global variable |
