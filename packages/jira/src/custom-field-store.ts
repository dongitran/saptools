...
import type { CustomFieldSnapshot, NormalizedCustomFieldSchema, PinnedCustomFieldConfig } from "./custom-fields.js";
...
const schemaShape = z.object({ custom: z.string().nullable(), customId: z.number().int().optional(), items: z.string().nullable(), type: z.string() });
const snapshotSchema = z.object({
  version: z.literal(1), cloudId: z.string(), cloudName: z.string(), discoveredAt: z.string(), totalFromApi: z.number(), fetched: z.number(),
  fields: z.array(z.object({ id: z.string(), key: z.string(), name: z.string(), custom: z.boolean(), orderable: z.boolean(), navigable: z.boolean(), searchable: z.boolean(), clauseNames: z.array(z.string()), schema: schemaShape })),
});
const pinnedSchema = z.object({
  version: z.literal(1), cloudId: z.string(), cloudName: z.string(), updatedAt: z.string(),
  fields: z.array(z.object({ id: z.string(), name: z.string(), schema: schemaShape })),
});

export interface CustomFieldStoreOptions { readonly homeDir?: string; }

export function jiraCloudDataDirectory(cloudId: string, options: CustomFieldStoreOptions = {}): string {
  return join(options.homeDir ?? homedir(), SAPTOOLS_DIR_NAME, JIRA_DIR_NAME, CLOUDS_DIR_NAME, safeCloudIdSegment(cloudId));
}
...
export async function readCustomFieldSnapshot(cloudId: string, options: CustomFieldStoreOptions = {}): Promise<CustomFieldSnapshot | null> {
  try {
...
export async function writeCustomFieldSnapshot(snapshot: CustomFieldSnapshot, options: CustomFieldStoreOptions = {}): Promise<void> {
  await writePrivateJson(customFieldSnapshotPath(snapshot.cloudId, options), snapshot);
}

export async function readPinnedCustomFields(cloudId: string, options: CustomFieldStoreOptions = {}): Promise<PinnedCustomFieldConfig | null> {
  try {
...
export async function writePinnedCustomFields(config: PinnedCustomFieldConfig, options: CustomFieldStoreOptions = {}): Promise<void> {
...
function sanitizeSnapshot(value: z.infer<typeof snapshotSchema>): CustomFieldSnapshot {
  return { ...value, fields: value.fields.map((field) => ({ ...field, schema: sanitizeSchema(field.schema) })) };
}
...
function sanitizeSchema(value: z.infer<typeof schemaShape>): NormalizedCustomFieldSchema {
  return { custom: value.custom, items: value.items, type: value.type, ...(value.customId === undefined ? {} : { customId: value.customId }) };
}
