...
const DASHBOARDS_URL = process.env.CF_LOG_SEARCH_FAKE_DASHBOARDS_URL ?? "";
const WORKING_USERNAME = process.env.CF_LOG_SEARCH_FAKE_DASHBOARDS_USERNAME ?? "fake-dashboards-user";
const WORKING_PASSWORD = process.env.CF_LOG_SEARCH_FAKE_DASHBOARDS_PASSWORD ?? "fake-dashboards-password";
const MULTI_INSTANCE = process.env.CF_LOG_SEARCH_FAKE_CF_MULTI_INSTANCE === "1";
...
const NO_SESSION = process.env.CF_LOG_SEARCH_FAKE_CF_NO_SESSION === "1";
...
const SLOW_MS = Number(process.env.CF_LOG_SEARCH_FAKE_CF_SLOW_MS ?? "0");
if (SLOW_MS > 0 && cmd === "auth") {
  await new Promise((resolve) => setTimeout(resolve, SLOW_MS));
}

function trace(entry) {
  const file = process.env.CF_LOG_SEARCH_FAKE_CF_TRACE_FILE;
  if (file) {
...
if (cmd === "curl") {
  ...
  // CF_LOG_SEARCH_FAKE_CF_KEY1_BROKEN makes EVERY key fail, to exercise the
  // app-binding fallback. CF_LOG_SEARCH_FAKE_CF_ONLY_KEY2_WORKS makes only key1
  // fail, so a test can prove key2 is tried before key1.
  const onlyKey2Works = process.env.CF_LOG_SEARCH_FAKE_CF_ONLY_KEY2_WORKS === "1";
  const isKey = binding?.type === "key";
  const broken = isKey
    ? onlyKey2Works
      ? binding.name !== "key2"
      : process.env.CF_LOG_SEARCH_FAKE_CF_KEY1_BROKEN === "1"
    : false;
