...
function cacheOptions(): { readonly cliName: string; readonly saptoolsRoot?: string } {
...
export function registerCredentialCommands(program: Command): void {
  const credential = program.command("credential").description("inspect or remove the cached Cloud Logging dashboards credentials (the secret itself is never shown)");

  const list = credential.command("list").description("list cached credentials by target, instance, source, endpoint and expiry — never the username or password");
