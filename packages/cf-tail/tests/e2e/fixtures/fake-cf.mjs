...
function getScenarioPath() {
  const path = process.env["CF_TAIL_FAKE_SCENARIO"];
  if (!path) {
    fail("Missing CF_TAIL_FAKE_SCENARIO");
  }
  return path;
}
...
async function logInvocation(command, args, state) {
  const path = process.env["CF_TAIL_FAKE_LOG_PATH"];
  if (!path) {
    return;
  }
  await mkdir(dirname(path), { recursive: true });
...
function renderAppsTable(apps) {
  const header = "name                requested state   instances   processes";
  const rows = apps.map((app) => {
    const name = String(app.name ?? "").padEnd(20);
    const state = (app.requestedState ?? "started").padEnd(18);
    const instances = String(app.instances ?? "1/1").padEnd(12);
    const processes = app.processes ?? "web:1/1";
    return `${name}${state}${instances}${processes}`;
  });
  return `${header}\n${rows.join("\n")}\n`;
}
...
async function main() {
  ...
  if (command === "apps") {
    ...
    process.stdout.write(renderAppsTable(space.apps ?? []));
    return;
  }
