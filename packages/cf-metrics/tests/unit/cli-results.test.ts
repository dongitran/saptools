...
describe("result show", () => {
  it("prints the saved rows in the requested format", async () => {
    ...
    await buildTestProgram().parseAsync(["node", "cf-metrics", "result", "show", "deadbeef", "--format", "json"]);
    expect(JSON.parse(output.text())).toEqual([{ NAME: "container.cpu.usage" }]);
  });
  ...
  it("prints one row's full JSON when --row is given", async () => {
    vi.mocked(resultStore.readResultSession).mockResolvedValue({
      ...
      ttlMinutes: 10_080,
      command: "names",
      rows: [{ NAME: "container.cpu.usage" }, { NAME: "container.memory.usage" }],
    });
    const output = captureStdout();
    await buildTestProgram().parseAsync(["node", "cf-metrics", "result", "show", "deadbeef", "--row", "2"]);
    expect(JSON.parse(output.text())).toEqual({ NAME: "container.memory.usage" });
  });

  it("rejects a non-numeric --row before touching the store", async () => {
    const program = buildTestProgram();
    await expect(program.parseAsync(["node", "cf-metrics", "result", "show", "deadbeef", "--row", "abc"])).rejects.toThrow();
  });
});
...
describe("result list/prune/clear", () => {
  it("lists active refs as a table", async () => {
    ...
    const output = captureStdout();
    await buildTestProgram().parseAsync(["node", "cf-metrics", "result", "list"]);
