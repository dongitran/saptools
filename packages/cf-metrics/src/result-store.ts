...
function resolveRef(value: string | undefined): string {
  ...
  if (!RESULT_REF_PATTERN.test(ref)) {
    throw new CfMetricsError("CONFIG", "Invalid saved result ref");
  }
  return ref;
}
...
function resolveTtl(value: number | undefined): number {
  ...
  if (!Number.isSafeInteger(ttl) || ttl <= 0) {
    throw new CfMetricsError("CONFIG", "Result TTL must be a positive safe integer");
  }
  return ttl;
}
 ...
 * (a manifest written by a newer cf-metrics, or a partial write worth keeping
 * for inspection). Collapsing all three into one "missing" answer is what used
 * to make a nominally read-only `result list` destroy saved results.
 */
...
function readFailureError(ref: string, failure: SessionReadFailure, saptoolsRoot?: string): CfMetricsError {
  const path = manifestPath(ref, saptoolsRoot);
  switch (failure) {
    case "absent":
      return new CfMetricsError("RESULT_NOT_FOUND", "Saved result not found or expired");
    case "unreadable":
    ...
    case "unrecognized":
      return new CfMetricsError(
        "RESULT_UNREADABLE",
        `Saved result "${ref}" is not in a format this version of cf-metrics understands. ` +
          `It has been left in place at ${path} — a newer cf-metrics may be able to read it.`,
      );
  }
}
...
export async function createResultSession(
  ...
  if (Buffer.byteLength(serialized) > (options.maxBytes ?? MAX_RESULT_STORE_BYTES)) {
    throw new CfMetricsError("CONFIG", "Saved result exceeds the storage limit");
  }
...
export async function readResultSession(ref: string, options: ResultStoreOptions = {}): Promise<ResultSession> {
  ...
  if (expiry <= now) {
    throw new CfMetricsError("RESULT_NOT_FOUND", "Saved result not found or expired");
  }
  return read.session;
}
