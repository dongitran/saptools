...
Every command first checks npm for a newer `@saptools/cf-events` (at most once an hour, one small request
...
```text
cf-events: updating 0.3.0 -> 0.4.0 ...
cf-events: updated to 0.4.0; re-running the command
```

If the install cannot complete, one stderr line gives the manual command and the command runs on the
installed version; that version is not retried for a day. `cf-events self-update` forces the check and
install now; `cf-events self-update --check` only reports.
...
| `CF_EVENTS_AUTO_UPDATE` | same values, this CLI only; wins over the global variable |
