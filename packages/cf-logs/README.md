...
Every command first checks npm for a newer `@saptools/cf-logs` (at most once an hour, one small request
...
```text
cf-logs: updating 0.7.0 -> 0.8.0 ...
cf-logs: updated to 0.8.0; re-running the command
```

If the install cannot complete, one stderr line gives the manual command and the command runs on the
installed version; that version is not retried for a day. `cf-logs self-update` forces the check and
install now; `cf-logs self-update --check` only reports.
...
| `CF_LOGS_AUTO_UPDATE` | same values, this CLI only; wins over the global variable |
