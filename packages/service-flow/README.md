...
- Index preparation remains deterministic and sequential. Each source file and `package.json` is read once into a repository-scoped immutable snapshot, and all TypeScript parser entrypoints share one lazy AST for that file. Publication uses one savepoint per repository; structured prepared-fact failures are contained at the individual fact, omitted with site-aware diagnostics, and do not discard valid sibling facts. Unexpected repository failures still roll back that repository, while the final package-invalidation/materialization pass remains workspace-atomic.
...
- Event receiver analysis structurally proves CAP-connect declarations and reaching assignments, including later assignment, destructuring, control-flow nesting, formatter-split chains, and one direct helper-return hop. A missing lexical binding remains a visible lower-confidence fact rather than being treated as proof of a non-CAP receiver. Mixed assignments remain unproven; proven non-CAP declarations plus observed socket, stream, and desktop receiver names are excluded. Static event-name identity and exact name-only subscriber traversal remain available across an unproven propagation receiver; the receiver uncertainty stays explicit in confidence and evidence.
...
- Equal template skeletons can appear under `--dynamic-mode candidates` as `EVENT_SHAPE_CANDIDATE_SUBSCRIBER` edges only when their hole layout is identical and at least one literal span is eight characters or longer. These edges are always dynamic and non-authoritative. Strict output reports how many candidates it hid and points to the opt-in flag; an excessive link-time expansion is refused with an explicit diagnostic rather than silently truncated.
- Repository environment declarations are opt-in allowlisted facts. The schema-14 analyzer reads configured `eventEnvironmentKeys` from `nodemon.json`, `.env`, `manifest.yml`, and `mta.yaml`; the shipped default is empty. `index --event-environment-key <KEY>` may be repeated to override the configured list for one invocation. Facts retain dev/deployment provenance and support only `toUpperCase()`/`toLowerCase()`. Shape candidates sharing an environment key pair only at equal deployment values; missing or conflicting values fail closed instead of producing a Cartesian product. Adjacent non-allowlisted keys are never persisted, and doctor reports configured keys with zero matches.
...
```json
{
  "eventEnvironmentKeys": ["REGION_CODE", "TENANT_CODE"]
}
```
...
Repeat `--event-environment-key` on `index` to replace and persist the
configured list for this and later index invocations:

```bash
service-flow index --workspace /path/to/workspace \
  --event-environment-key REGION_CODE \
  --event-environment-key TENANT_CODE
```

> [!IMPORTANT]
> Package `0.1.75` uses schema 15 and advances fact compatibility to analyzer `0.1.75-facts.1`. Decorator constant identity, receiver classification, and event environment semantics changed, so every existing workspace requires:
>
> ```bash
