...
const legacyAnalyzer = '0.1.66-facts.1';
...
function seedLegacySnapshot(db: Db, dbPath: string): void {
  ...
  db.prepare(`INSERT INTO repositories(
    ...
    'indexed', 'legacy-fingerprint', 6, 5, legacyAnalyzer,
  );
...
function downgradeFromV13(db: Db): void {
  db.exec(`
...
async function createLegacyDatabase(version: 11 | 12): Promise<string> {
  const dbPath = await databasePath(`schema-v${version}`);
  const db = openDatabase(dbPath);
  seedLegacySnapshot(db, dbPath);
  downgradeFromV13(db);
  if (version === 11) downgradeFromV12(db);
  db.close();
  return dbPath;
}
...
async function verifyV12Migration(): Promise<void> {
  ...
  expect(migrated.prepare(`SELECT package_public_surface_json surface,
    ...
    .toEqual({
      surface: null,
      staleReason: 'schema_v13_fact_provenance_requires_reindex',
    });
...
async function verifyFutureSchema(): Promise<void> {
  const dbPath = await databasePath('future-v14');
  ...
  expect(() => migrate(future))
    .toThrow('Unsupported future service-flow schema version 14');
  expect(schemaVersion(future)).toBe(14);
  future.close();
}

describe('schema 13 lifecycle and provenance migration', () => {
  ...
  it('rejects schema version 14 before current-table inspection or migration', verifyFutureSchema);
});
