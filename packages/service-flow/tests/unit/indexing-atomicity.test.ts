...
import { factLifecycleDiagnostic } from '../../src/db/fact-lifecycle.js';
...
`);
    ...
    expect(migrated.prepare(`SELECT graph_stale_reason reason,
      ...
      reason: 'schema_v14_event_surface_requires_reindex', analyzer: '0.1.65',
    });
    ...
    expect(migrated.prepare('SELECT graph_stale_reason reason FROM repositories WHERE id=1')
      .get()?.reason).toBe('schema_v14_event_surface_requires_reindex');
