...
`);
  ...
  it('prefers indexed operation evidence over legacy remote entity classification for operation-like sends', async () => {
    ...
    markRepositoryCurrent(db, adminRepo, 'admin-api');
    markRepositoryCurrent(db, facadeRepo, 'facade-api');
    ...
    const bindingId = insertOwnerlessBinding(db, facadeRepo, {
      variableName: 'adminService', alias: 'admin-api',
      servicePathExpr: '/AdminService', sourceFile: 'srv/facade.ts',
      sourceLine: 3, startOffset: 10, endOffset: 20,
    });
    insertOwnerlessCall(db, facadeRepo, {
      callType: 'remote_entity_mutation', bindingId, method: 'POST',
      operationPathExpr: '/refreshCache()', queryEntity: 'refreshCache',
      sourceFile: 'srv/facade.ts', sourceLine: 4,
      startOffset: 100, endOffset: 120,
      evidence: { parser: 'legacy_remote_entity_fixture' },
    });
    linkWorkspace(db, workspaceId);
...
describe('service-flow OData entity and operation precedence hardening', () => {
  it('keeps entity key and media placeholders terminal while resolving explicit operations', async () => {
    ...
    markRepositoryCurrent(db, documentsRepo, 'documents-api');
    markRepositoryCurrent(db, callerRepo, 'workflow-api');
    ...
    const bindingId = insertOwnerlessBinding(db, callerRepo, {
      variableName: 'documentService', alias: 'documents-api',
      servicePathExpr: '/DocumentService', sourceFile: 'srv/handler.ts',
      sourceLine: 3, startOffset: 10, endOffset: 20,
    });
    const calls = [
      ['remote_entity_mutation', 'POST', '/DocumentAttachment', 'DocumentAttachment'],
      ['remote_entity_media', 'PUT', '/DocumentAttachment(${attachment.ID})/file', 'DocumentAttachment'],
      ['remote_query', 'GET', '/DocumentAttachment(${attachmentID})', 'DocumentAttachment'],
      ['remote_entity_mutation', 'POST', '/refreshCache()', 'refreshCache'],
    ] as const;
    calls.forEach(([callType, method, operationPathExpr, queryEntity], index) => {
      const startOffset = 100 + index * 30;
      insertOwnerlessCall(db, callerRepo, {
        callType, bindingId, method, operationPathExpr, queryEntity,
        sourceFile: 'srv/handler.ts', sourceLine: index + 4,
        startOffset, endOffset: startOffset + 20,
