...
Every command first checks npm for a newer `@saptools/cf-xsuaa` (at most once an hour, one small request
...
```text
cf-xsuaa: updating 0.2.0 -> 0.3.0 ...
cf-xsuaa: updated to 0.3.0; re-running the command
```

If the install cannot complete, one stderr line gives the manual command and the command runs on the
installed version; that version is not retried for a day. `cf-xsuaa self-update` forces the check and
install now; `cf-xsuaa self-update --check` only reports.
...
| `CF_XSUAA_AUTO_UPDATE` | same values, this CLI only; wins over the global variable |
