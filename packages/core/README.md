...
| `cloud-logging/target`, `cloud-logging/instance-discovery` | Resolve region/org/space from `--region`/`--org`/`--space` or the ambient `cf target` session (`resolveTarget`, `printResolvedTarget`), and auto-discover the one `cloud-logging` service instance in a space via the v3 API, failing closed on none or several (`discoverServiceInstance`, `listCloudLoggingInstances`). Every function here takes a `CloudLoggingCfExecutor` — a thin adapter each consumer builds over its own `cf.ts`. |
